  ...
  return (
    <SectionShell id="tech" variant={variant} className="!py-16 md:!py-20">
      ...
      <motion.div
        ...
        className={`mt-10 divide-y ${isDark ? "divide-white/10 border-white/10" : "divide-slate-200 border-slate-200"} border-y`}
      >
        {techStack.map((cat) => (
          <motion.article
            key={cat.category}
          ...
          >
            ...
            <p
              className={`text-sm leading-relaxed ${isDark ? "text-slate-400" : "text-muted"}`}
            >
              {cat.description}
            </p>
            <p
              className={`text-xs font-medium tracking-wide md:text-right ${isDark ? "text-primary" : "text-primary/80"}`}
            >
              {cat.items.join(" · ")}
            </p>
          </motion.article>
